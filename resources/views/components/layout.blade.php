        ...
        <nav>
            ...
            {{-- Dropdown Menu --}}
                <div x-show="open" @click.outside="open=false" class="bg-white shadow-lg absolute top-10 right-0 rounded-lg overflow-hidden font-light">
                    ...
                    {{-- Check if user is a admin and provides access to admin dashboard --}}
                    @if (auth()->user()->is_admin)
                        <a href="{{ route('admin') }}" class="block w-full text-left hover:bg-slate-100 pl-4 pr-8 py-2">Admin Panel</a>
                    @endif
                    <a href="{{ route('dashboard') }}" class="block w-full text-left hover:bg-slate-100 pl-4 pr-8 py-2">Dashboard</a>
